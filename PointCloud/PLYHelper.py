...
def write_PLY(fn, disp, Q, mask=None, color=None, binary=True):
    """
    ...
    d  = disp.reshape((1, -1,))
    hg = np.ones((1, d.shape[1]), dtype=np.float32)
    ...
    if ( mask is not None ):
        m = np.logical_and( m, mask.reshape((1,-1)) ) 

    coor = np.stack( [ \
        x[m], y[m], d[m], hg[m] ], axis=-1 ).transpose()
